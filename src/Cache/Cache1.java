...
public class Cache1 extends Thread {
    ...
    public Cache1(int num, String q) {
    ...
    public void fn(int i, String q) {
        ...
        switch (http_method) {
            case "GET":
                if (id == "") {
                    System.out.println("Buscando los ultimos 10 registros de tipo '" + resource + "'");

                } else {
                    System.out.println("La hebra " + this.numHilo + " está buscando en '" + resource + "' el registro " + id);
                    ...
                    String estaEnCacheEstatico = cestatico.getEntryFromCache(id); // Buscar en el cache estático
                    if (estaEnCacheEstatico == null) { // Si no esta en el cache estatico
                        ...
                        else //si no esta en el cache estatico y este esta lleno entonces pasamos al dinamico 
                        {
                            ...
                            String result = lru_cache1.getEntryFromCache(id);// busco en el cache el id del request
                            if (result == null) { // MISS
                                System.out.println("MISS :(");
                                //total_miss++;
                                result = IndexService.getEntry(id);//sino lo encuentra en cache lo busca en el index
                                lru_cache1.addEntryToCache(id, result);// luego lo pongo en el cache
                            } else {
